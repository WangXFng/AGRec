...
class Metric(object):
    ...
    def hit_ratio(origin, hits):
        ...
        Note: This type of hit ratio calculates the fraction:
         (# retrieved interactions in the test.txt set / #all the interactions in the test.txt set)
        """
        total_num = 0
        for user in origin:
            items = list(origin[user].keys())
    ...
    #     """
    #     Note: This type of hit ratio calculates the fraction:
    #      (# users who are recommended items in the test.txt set / #all the users in the test.txt set)
    #     """
    #     hit_num = 0
    #     for user in hits:
    #         if hits[user] > 0:
...
def ranking_evaluation(origin, res, N):
    ...
    for n in N:
        ...
        if len(origin) != len(predicted):
            print('The Lengths of test.txt set and predicted set do not match!')
            exit(-1)
