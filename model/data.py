...
import pdb
import json
from model.prompt import sft_prompt, all_prompt
import numpy as np
import itertools
        ...
        def prefix_allowed_tokens_fn(batch_id, sentence):
            ...
            for i in range(len(reversed_sent)):
                if reversed_sent[i:i + len(sep)] == sep[::-1]:
                    result = list(self.allowed_tokens[i])
                    # return list(self.allowed_tokens[i])

            if len(result) < 20:
                complementary_ids = list(i for i in range(20 - len(result)))
                result.extend(complementary_ids)
...
class SeqRecDataset(BaseDataset):
    ...
    def random_sampling_one_permutation(self, strings):
        return ''.join(random.choice(list(itertools.permutations(strings))))


    def _get_interactions_from_train_data(self):
        return self.remapped_inters2
    ...
    def _process_valid_data(self):
        ...
        for uid in self.remapped_inters:
            ...
            one_data['user_id'] = int(uid) # only for training
            one_data["inters"] = self.his_sep.join(history)
            inter_data.append(one_data)
        ...
        if self.sample_valid:
            ...
            for i in range(len(self.inter_data)):
                ...
                for prompt_id in prompt_ids:
                    ...
                    self.valid_text_data.append({"input_ids": input, "user_id":d["user_id"], "labels": output})
        else:
    ...
    def __getitem__(self, index):
        ...
        return dict(input_ids=input, user_id=d['user_id'], labels=output)
