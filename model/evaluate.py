...
def get_topk_results_and_logs(predictions, scores, targets, k, all_items=None, user_ids=None):
    results = []
    logs = []
    B = len(targets)
    ...
    for b in range(B):
        ...
        for sorted_pred in sorted_pairs:
            if sorted_pred[0] not in all_items:
                break
        ...
        if sum(one_results) > 0:
            logs.append(str(user_ids[b].cpu().item()) + '\t' + str(one_results[:10]))
